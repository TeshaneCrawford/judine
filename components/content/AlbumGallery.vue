  ...
  <div>
    <section v-if="images" ref="dropZoneRef" class="relative h-screen gap-[22px] p-4">
      <USlideover v-model="isOpen" class="flex items-center justify-center" side="left">
        <AdminLogin class="z-50 bg-gray-800 rounded-md" @close-login="isOpen = false" />
        <UButton icon="i-heroicons-x-mark" class="absolute right-4 top-4" @click="isOpen = false" />
      </USlideover>

      <BottomMenu class="bottom-menu">
        <template #logo>
          <NuxtImg src="/img/logo.jpg" width="29" height="20" />
        </template>
        <template #description>
          <div class="flex gap-x-4 items-center">
            ...
            <NuxtLink to="https://github.com/TeshaneCrawford/judine" target="blank" class="flex items-center">
...
<template>
  <div>
    <h3>Album</h3>
    <FancyBox>
      <p class="not-prose grid grid-cols-2 sm:grid-cols-4 gap-2 space-y-2">
      <NuxtImg
